...
; This is not currently installed and I'm not bothered about the font
...
(defun casey-big-fun-c-hook ()
  ...
  (defun casey-header-format ()
     ...
     (insert "   $Date: $\n")
     (insert "   $Revision: $\n")
     (insert "   $Creator: Christopher O'Donnell $\n")
     (insert "   $Notice: (C) Copyright 2017 by Dipole Limited. All Rights Reserved. $\n")
     (insert "   ======================================================================== */\n")
     (insert "\n")
     (insert "#define ")
     (push-mark)
  ...
  (define-key c++-mode-map "\C-s" 'casey-save-buffer)
...
(defun casey-big-fun-text-hook ()
  ...
  ; Prevent overriding of ctrl-s
  (define-key text-mode-map "\C-s" 'casey-save-buffer)
  )
...
; Commenting this out might break something else
